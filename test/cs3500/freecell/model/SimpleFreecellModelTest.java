...
public class SimpleFreecellModelTest {
  ...
  public void testIsDeckValidExceptions() {
    initializeData();
    // not 52 cards
    model.isDeckValid(invalidDeck);
    validDeck.remove(51);
    // deck has 52 cards but now has a duplicate
    validDeck.add(new Card(4, Suit.SPADES));
    model.isDeckValid(validDeck);

    initializeData();
    // deck has 52 cards but one of the cards is invalid
    validDeck.remove(51);
    validDeck.add(new Card(0, Suit.HEARTS));
    model.isDeckValid(validDeck);
  }
  ...
  public void testIsMoveValidExceptions() {
    ...
    // values are one less but the suits aren't the same color
    model.isMoveValid(card2, fiveS);
    // this is a valid move
    model.isMoveValid(fourD, fiveS);
